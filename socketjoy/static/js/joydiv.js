...
(function (JoydivModule) {
    var Direction = (function () {
        function Direction(offset, dim=2) {
            this.dim = dim;
            if (this.dim == 1){
                this.offset = {
                    y: offset.y
    ...
    var Trigger = (function () {
        function Trigger(options) {
            ...
            this.rootElement = options.element;
            this.trackpad = querySelector(this.rootElement, '.trigger-trackpad');
            this.tracker = querySelector(this.trackpad, '.trigger-tracker');

            var onTrackerScreenPos = function (e, origin) {
                // For now these can only be vertical
                // var x = 2 * (e.screenX - origin.x) / _this.rootElement.clientWidth;
                var y = 2 * (e.screenY - origin.y) / _this.rootElement.clientHeight;
                // _this.tracker.style.left = (x + 0.5) * 100 + "%";
            ...
            (function () {
                ...
                var unmount = function () {
                    origin = null;
                    // _this.tracker.style.left = "50%";
                    _this.tracker.style.top = _this.options.topHome;
                    _this.removeVote('tracker-mouse');
                };
                document.addEventListener('mousemove', function (e) {
                    if(origin) {
        ...
        Trigger.prototype.each = function (obj, foo) {
            if(obj.length === +obj.length) {
                for(var i = 0, l = obj.length; i < l; i++) {
                    foo(obj[i], i, obj);
                }
        ...
        Trigger.prototype.map = function (obj, foo) {
            var acc = [];
            this.each(obj, function (val, key) {
                acc.push(foo(val, key, obj));
            });
        ...
        Trigger.prototype.removeVote = function (voter) {
            if(voter in this.votes) {
                delete this.votes[voter];
                this.changed();
            }
        ...
        Trigger.prototype.changed = function () {
            var event = document.createEvent('Event');
            event.initEvent('trigger-changed', true, false);
            event.detail = {
                trigger: this
            };
            this.rootElement.dispatchEvent(event);
        };
        Trigger.prototype.getNoneDirection = function () {
            return new Direction({
                // x: 0,
                y: 0
            }, dim = 1);
        };
        Trigger.prototype.getRawOneDirection = function () {
            var acc = {
                // x: 0,
                y: 0
            };
        ...
        Trigger.prototype.getOneDirection = function () {
            return this.getPostProcessed(this.getRawOneDirection());
        };
        Trigger.prototype.getPostProcessed = function (direction) {
            direction = new Direction(direction.offset, dim=1);
            // if(this.options.clampX) {
            //     var m = Math.abs(direction.offset.x);
            //     if(this.options.clampX < m) {
            //         direction.offset.x /= m;
            ...
            //     }
            // }
            var newDir = new Direction(direction.offset, dim = 1);
            if(this.options.flipY) {
                newDir.offset.y *= -1;
                // newDir.angle = (Math.PI * 3 - newDir.angle) % (Math.PI * 2);
            }
            // Exclude negative values
            if(this.options.positiveOnly && newDir.offset.y < 0){
                newDir.offset.y = 0;
            }
            return newDir;
        };
